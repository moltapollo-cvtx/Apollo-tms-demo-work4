...
import { FileText, DownloadSimple, Eye, Lock } from "@phosphor-icons/react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { DataTable, type Column } from "@/components/ui/data-table";
import type { Document, OrderWithDetails } from "@/types";
...
export default function OrderDocumentsTab({ order }: OrderDocumentsTabProps) {
  const documents = (order.documents || []) as Document[];
    ...
    <div className="space-y-4">
      ...
      <Card className="p-4">
        <DataTable
          data={documents}
          columns={columns}
          searchPlaceholder="Search files, mime types, or document types..."
          emptyState={{
            icon: <FileText className="h-full w-full" />,
            title: "No documents attached",
            description: "Upload BOL, POD, and rate confirmations to complete this load record.",
          }}
          defaultPageSize={6}
